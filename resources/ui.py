...
class backgroundTasks():
    ...
    async def checker_status(self):
        if self.checker_enabled:
            return ("Checker Status:    " + Back.GREEN + "Running" + Style.RESET_ALL)
        else:
            return ("Checker Status:    " + Back.RED + "Stopped" + Style.RESET_ALL)

    # Main process of the sniper
    async def checker(self):
        while True:
            buyList = await self.api_handler.check_stock_test() # This is a testing function only
            await self.buy_item(buyList) if buyList else print("No items to buy.") # This is a testing statement only
            sleep_duration = random.uniform(5, 8)
            await asyncio.sleep(sleep_duration)
            
    
    # Check Login Status on start
    async def check_login_status_on_start(self):
        status_code = await self.api_handler.is_session_expired()
        if status_code == 0:
            return ("Login Status:      " + Back.GREEN + "Logged in" + Style.RESET_ALL)
        else:
            return ("Login Status:      " + Back.RED + "Not logged in" + Style.RESET_ALL)
